...
rofl::line_strip::object<A, B>::object(
    ...
    : renderer_(_renderer), style_(params.style()), color_(params.color()),
      points_(params.points().begin(), params.points().end()),
      vertex_declaration_(renderer_.get().create_vertex_declaration(
...
template <typename A, typename B> object<A, B>::~object() = default;
}

template <typename A, typename B>
rofl::line_strip::object<A, B>::object(object &&) noexcept = default;

template <typename A, typename B>
rofl::line_strip::object<A, B> &
rofl::line_strip::object<A, B>::operator=(object &&) noexcept = default;

template <typename A, typename B>
void rofl::line_strip::object<A, B>::push_back(
...
template <typename A, typename B>
void rofl::line_strip::object<A, B>::pop_back() {
  points_.pop_back();
  regenerate_vb();
}

template <typename A, typename B> void rofl::line_strip::object<A, B>::clear() {
  points_.clear();
  regenerate_vb();
}

template <typename A, typename B>
void rofl::line_strip::object<A, B>::draw(
  ...
  sge::renderer::vertex::scoped_buffer const vb_context(
      fcppt::make_ref(_render_context), fcppt::make_cref(*vertex_buffer));
...
template <typename A, typename B>
void rofl::line_strip::object<A, B>::regenerate_vb() {
      ...
      fcppt::optional::assign(
          ...
          renderer_.get().create_vertex_buffer(
              sge::renderer::vertex::buffer_parameters(
                  ...
                  sge::renderer::vf::dynamic::make_part_index<format,
                                                              format_part>(),
                  sge::renderer::vertex::count(points_.size() +
                                               (style_ == style::loop ? 1 : 0)),
  ...
  for (auto const &v : points_) {
    sge::renderer::vf::set_proxy(*vb_it, sge::renderer::vf::labels::pos{}, v);

    sge::renderer::vf::set_proxy(*vb_it, sge::renderer::vf::labels::color{},
                                 color_);
  ...
  case style::no_loop:
    break;
  case style::loop:
    sge::renderer::vf::set_proxy(*vb_it, sge::renderer::vf::labels::pos{},
                                 points_.front());
...
template <typename A, typename B>
void rofl::line_strip::object<A, B>::back(point const &p) {
  points_.back() = p;
  regenerate_vb();
}
